...
const formantWorker = new Worker(new URL("./formantWorker.js", import.meta.url), {
  type: "module",
});
...
function addFormantsToHistory(f0: number, f1: number, f2: number, f3: number, f4: number) {
  formantHistory.push({
...
function drawAxisTicks(
  ...
  const { minTime, now, timeWindow, minFreq, maxFreq, xFromTime, yFromFreq, width, height } = opts;
...
async function start() {
  try {
    ...
    const getFrequency = (index: number) => index * (maxFrequency / bufferLength);
    ...
    formantWorker.onmessage = (event: MessageEvent<WorkerResponse>) => {
      ...
      if (message.type === "init") {
        ...
        if (message.status === "success" && message.formants && message.pitch !== undefined) {
    ...
    const drawSpectrum = () => {
      ...
      if (showFFTSpectrum.checked) {
        ...
        const maxMagnitude = Math.max(...spectrum.map((value) => Math.abs(value)), 10);
        ...
        for (let i = 0; i < bufferLength; i += 1) {
          ...
          const y = spectrumCanvas.height - (logMagnitude / logMaxMagnitude) * spectrumCanvas.height;
    ...
    const drawLPCFilter = () => {
      if (showLPCSpectrum.checked) {
        ...
        for (let i = 0; i < graphSize; i += 1) {
          ...
          const yPos =
            spectrumCanvas.height - (logResponse / logMaxResponse) * spectrumCanvas.height;
    ...
    const drawFormantHistory = () => {
      ...
      const xFromTime = (time: number) => ((time - minTime) / timeWindow) * width;
      ...
      const recentData = formantHistory.filter((entry) => entry.time >= minTime);
...
requireElement<HTMLButtonElement>("showSpectrumTabBtn").addEventListener("click", () => {
  spectrumCanvas.style.display = "block";
  historyCanvas.style.display = "none";
});

requireElement<HTMLButtonElement>("showHistoryTabBtn").addEventListener("click", () => {
